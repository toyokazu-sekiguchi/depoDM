...
class Deposit:
    ...
    def SetParams(self,epspath,intype):
        ...
        if(self.intype==1):
            if(self.verbose>0): print(" injection type: annihilation")
            self.pow = 6
        elif(self.intype==2):
            if(self.verbose>0): print(" injection type: decay")
            self.pow = 3
        else:
            print("error: only annihilation or decay is supported")
            sys.exit(1)
    ...
    def CheckEpsData(self):
        ...
        for n in range(1,self.nspecies):
            if(not((self.epsdata[n].z1out==self.z1out).all() and (self.epsdata[n].erg==self.erg).all())):
                ...
                sys.exit(1)
            if(not (self.epsdata[n].ch==self.ch).all()):
                print("error: channels in epsdata disagree")
                sys.exit(1)
        self.nz1out = len(self.z1out)
        self.nerg = len(self.erg)
        self.nch = len(self.ch)
