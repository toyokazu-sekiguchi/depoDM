...
double *nurho;
double lnlamstart, dlnlam;
//injection
double *Xion, *Xexc, *Xheat;
double lnz1start, dlnz1;
//injection
...
void init_mnu() {
  ...
  dlnlam = log(MNU_MAX/MNU_MIN)/(NMNU-1);
...
double rho_mnu(double lnlam) {
  if (lnlam < lnlamstart) {
    return 1.;
  } else if (lnlam > lnlamstart + dlnlam * NMNU) {
    return exp(lnlam) * 0.317322;
  } else {
    return rec_interp1d(lnlamstart, dlnlam, nurho, NMNU, lnlam);
  };

}

//injection
void init_injection(double INJion[63], double INJexc[63], double INJheat[63]) {

  unsigned i;

  lnz1start = log(Z1MIN);
  dlnz1 = log(Z1MAX/Z1MIN)/(NZ1-1);

  Xion = create_1D_array(NZ1);
  Xexc = create_1D_array(NZ1);
  Xheat = create_1D_array(NZ1);
...
//injection
double rec_Xion(REC_COSMOPARAMS *param, double z) {
  
  double lnz1;
  lnz1 = log(z+1.);
  if(lnz1 < lnz1start) { // not sure what how we can deal with this regime...
    return Xion[0];
  } else if (lnz1 > lnz1start+dlnz1*(NZ1-1)) {
    return Xion[NZ1-1];
  } else {
    return rec_interp1d(lnz1start, dlnz1, Xion, NZ1, lnz1);
  };
  
}

double rec_Xexc(REC_COSMOPARAMS *param, double z) {
  ...
  if(lnz1 < lnz1start) { // not sure what how we can deal with this regime...
    ...
    return rec_interp1d(lnz1start, dlnz1, Xexc, NZ1, lnz1);
  };
  
}

double rec_Xheat(REC_COSMOPARAMS *param, double z) {
  ...
  if(lnz1 < lnz1start) { // not sure what how we can deal with this regime...
    ...
    return rec_interp1d(lnz1start, dlnz1, Xheat, NZ1, lnz1);
  };
...
double rec_HubbleConstant(REC_COSMOPARAMS *param, double z) {
   ...
   rnu = 0.;
   for(j=0; j<3; j++) {
     lnlam = log(param->mnu[j] * 1.16045e4 / (1.+z) / (param->T0 * 0.713765856));
     rnu += rho_mnu(lnlam);
   };
   rho_i[4] *= rnu/3;
...
void rec_get_xe_next2_HTm(int func_select, REC_COSMOPARAMS *param, double z_in, double xe_in, double Tm_in, double *xe_out, double *Tm_out,
    ...
    //injection
    dTmdlna += Yheat/kBoltz*2./3/(1.+xe_in+param->fHe);
    //injection
